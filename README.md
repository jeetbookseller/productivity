...
Productivity Hub is a web app that runs in your browser — no app store required. Visit the URL, log in, and your data is stored in the cloud (Supabase) and synced across your devices. It can optionally be installed to your home screen on iOS and Android like a native app. It also works offline after the first visit, with changes syncing automatically when you're back online. The app's own code is pre-built and served as static files from GitHub Pages; Supabase provides the hosted backend for authentication and data storage.
